...
├── lpdm/configs/                      # Configuration file directory
│   ├── test/
│   │   ├── denoise.yaml          # LPDM inference config (paths, diffusion steps, etc.)
│   └── train/
│       └── lpdm_lol.yaml         # LPDM training config
├── lpdm/external/                # LPDM external dependencies (CLIP/taming-transformers)
├── lpdm/checkpoints              #pretrained weight
└── loldataset/
│    ├── eval15/                  # Validation set (15 pairs)
│    │   ├── high/                # Normal-light ground truth
│    │   ├── low/                 # Original low-light images
...
├── evaluate.py                   #evalute enhanced images quality by SSIM,PSNR, .etc
├──resize.py                      #resize the images dataset
