...
def load_model_from_config(config, ckpt, verbose=False):
    ...
    try:
        ...
        torch.serialization.add_safe_globals([Timer])
    except ImportError:
        print("⚠️ pytorch_lightning未安装，跳过Timer类白名单添加")
    
    try:
        pl_sd = torch.load(ckpt, map_location="cpu", weights_only=True)
    except (pickle.UnpicklingError, RuntimeError):
        print("⚠️ weights_only=True加载失败，降级为weights_only=False（仅信任LPDM官方权重时安全）")
...
def load_model_given_name(name, device = torch.device('cpu')):
    ...
    model = load_model_from_config(config, f'{config_path[:-5]}.ckpt') # 权重文件与配置同名
...
parser.add_argument('-b', '--base_path', type=str, default='../configs/test/denoise.yaml', 
                    help='a string path to the test configuration file') 
parser.add_argument('-s', '--skip_large', action='store_true', 
                    help='a flag indicating whether to skip large images which are >2000 pixels in width or height') 
...
with torch.no_grad():
    for p_path, c_path  in tqdm(zip(pred_paths, cond_paths), total=len(pred_paths)):
        ...
        p = pil_to_tensor_in_range(Image.open(p_path)).unsqueeze(0)  # CNN增强图
        c = pil_to_tensor_in_range(Image.open(c_path)).unsqueeze(0)  # 原始低光图
